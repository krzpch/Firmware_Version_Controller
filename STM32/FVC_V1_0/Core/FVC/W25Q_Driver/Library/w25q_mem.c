 ...
 * @{
 */
extern QSPI_HandleTypeDef hqspi1;	///< Quad SPI HAL Instance
/// @}
...
W25Q_STATE W25Q_ReadStatusReg(u8_t *reg_data, u8_t reg_num) {
	...
	if (HAL_QSPI_Receive(&hqspi1, reg_data, HAL_QSPI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK) {
		return W25Q_SPI_ERR;
	}
...
W25Q_STATE W25Q_ProgramRaw(u8_t *buf, u16_t data_len, u32_t rawAddr) {
	...
	if (HAL_QSPI_Transmit(&hqspi1, buf, HAL_QSPI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK)
		return W25Q_SPI_ERR;
