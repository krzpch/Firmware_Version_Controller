from enum import IntEnum
from struct import pack, unpack
import numpy as np

server_address = 0

class data_types(IntEnum):
    TYPE_NACK   = 0
    TYPE_ACK    = 1
    TYPE_ID_REQ = 2
    TYPE_ID_RESP = 3
    TYPE_CLI_DATA = 4
    TYPE_PROGRAM_UPDATE_REQUEST = 5
    TYPE_PROGRAM_DATA = 6
    
starting_crc_value = 0xff

sfd = 0xAB

crc_table = np.array([])

def crc_calc(data_in: bytes):
    crc_out = starting_crc_value
    if data_in == b'':
        return 0xff
    size = len(data_in)
    num = 0
    crc_out &= 0xff
    while size != 0:
        crc_out = crc_table[crc_out ^ data_in[num]]
        size -= 1
        num += 1
    return crc_out

def serialize_packet(d_type: data_types._member_names_, dest_addr: int, data: bytes) -> bytes:
    packet = pack(">BHBBB", sfd, 7 + len(data), int(server_address), dest_addr, data_types(d_type).value)
    
    match d_type:
        case data_types.TYPE_ID_RESP:
            packet += pack(">B", data)
        case data_types.TYPE_CLI_DATA:
            packet += pack(">"+str(len(data))+"s", data)
        case data_types.TYPE_PROGRAM_DATA:
            packet += pack(">"+str(len(data))+"s", data)
        case data_types.TYPE_PROGRAM_UPDATE_REQUEST:
            packet += pack(">"+str(len(data))+"s", data)
        case other:
            pass
    
    packet = packet + pack(">B", crc_calc(packet))
    return packet

def deserialzie_packet(package: bytes):
    if crc_calc(package) != 0:
        return None
    
    data_len = len(package)
    if data_len > 7:
        data = unpack(">BHBBB"+ str(data_len-7) +"sB", package)
        (sfd, packet_len, src_ID, dest_ID, data_t, payload, crc) = data
    else:
        data = unpack(">BHBBBB", package)
        (sfd, packet_len, src_ID, dest_ID, data_t, crc) = data
        
    return data

def get_packet_len(initial_bytes: bytes) -> int:  
    data = unpack(">BHB", initial_bytes)
    
    if data[0] != sfd:
        return -1
    
    return data[1] - 4



